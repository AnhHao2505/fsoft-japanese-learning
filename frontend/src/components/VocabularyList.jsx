    ...
    <div className="card">
      <div className="card-header" style={{flexDirection: 'column', alignItems: 'flex-start', gap: '16px'}}>
        <div className="tabs">
          ...
          <button className="tab" onClick={() => navigate('/grammars')}>
            Bài Giảng Ngữ Pháp
          </button>
      ...
      <div className="card-body" style={{padding: '20px'}}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <h2 style={{ display: 'flex', alignItems: 'center', gap: '8px', margin: 0, color: 'var(--primary)' }}>
            <BookOpen size={24} /> Danh mục Từ Vựng (JPD316)
          </h2>
        ...
        {loading ? (
          ...
          <div className="lesson-list">
            {filteredLessons.map((lesson, idx) => (
              <Link to={`/vocabularies/${lesson.id}`} className="lesson-item" style={{ animationDelay: `${idx * 0.05}s`, animationFillMode: 'both', animationName: 'fadeInUp', animationDuration: '0.5s' }} key={lesson.id}>
